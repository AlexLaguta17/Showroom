...
urlpatterns = [
    ...
    path(
        "<int:showroom_pk>/cars/<int:car_pk>/", ShowroomCarViewSet.as_view(RUD_methods)
    ),
    path("<int:showroom_pk>/orders/", CarShowroomOrderViewSet.as_view(CR_methods)),
    path(
        "<int:showroom_pk>/orders/<int:order_pk>/",
        CarShowroomOrderViewSet.as_view(RUD_methods),
    ),
    path("<int:showroom_pk>/discounts/", ShowroomDiscountViewSet.as_view(CR_methods)),
    path(
        "<int:showroom_pk>/discounts/<int:discount_pk>/",
